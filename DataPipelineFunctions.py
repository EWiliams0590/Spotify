...
def create_artist_song_dict(files, N, filepath):
    """
    Parameters
    ----------
    ...
    counts_final.to_csv(filepath, index=False)
    final = counts_final.drop('Play Count', axis=1)
    # Convert final to dict
    artist_song_dict = final.groupby(by='artistName').agg(list).to_dict()['trackName']
